...
import { generateParfileContent, generateCommand } from "./utils/parfileGenerator";
...
function App() {
  const {
    config, setConfig, isLogfileSame, showAdvanced,
    handleOperationChange, handleDumpfileChange, handleLogfileSameChange,
    handleParallelChange, handleShowAdvancedToggle, handleConvertToImport
  } = useParfileConfig();
  ...
  const handleAiValidate = async () => {
    ...
    const contentToValidate = outputMode === "command" ? generateCommand(config, outputMode) : generateParfileContent(config);
      ...
      <main className="max-w-screen-2xl mx-auto py-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 lg:gap-x-6 lg:items-start space-y-6 lg:space-y-0">
          ...
          <div className="lg:col-span-5 bg-white shadow-md sm:rounded-lg p-5 space-y-6">
            ...
            <div className="border-b border-gray-200 pb-5">
              <PrimarySetupForm config={config} setConfig={setConfig} isLogfileSame={isLogfileSame} onLogfileSameChange={handleLogfileSameChange} onDumpfileChange={handleDumpfileChange}/>
            </div>
            <div className="border-b border-gray-200 pb-5">
          ...
          <div className="lg:col-span-4 space-y-6 lg:sticky top-20">
            <div className="bg-white shadow-md sm:rounded-lg">
              ...
              <div className="p-4 pt-0 space-y-4">
                {outputMode === "parfile" && <div className="relative"><textarea rows={12} readOnly value={generateParfileContent(config)} className="w-full p-2 font-mono bg-gray-100 border rounded-md" /><CopyButton contentToCopy={generateParfileContent(config)} /></div>}
                <div className="relative"><textarea rows={outputMode === "command" ? 18 : 5} readOnly value={generateCommand(config, outputMode)} className="w-full p-2 font-mono bg-gray-100 border rounded-md" /><CopyButton contentToCopy={generateCommand(config, outputMode)} /></div>
              </div>
          ...
          <div className="lg:col-span-3 lg:sticky top-20">
            <div className="bg-white shadow-md sm:rounded-lg">
              <div className="p-4"><h3 className="text-sm font-semibold mb-3">Advanced Validation</h3><button type="button" onClick={handleAiValidate} disabled={isInvalid || isAiValidating} className="w-full py-2 px-4 border shadow-sm rounded-md text-white bg-green-600 hover:bg-green-700 disabled:opacity-50">{isAiValidating ? "Validating..." : "Validate with AI"}</button></div>
              <div className="border-t border-gray-200 p-4 space-y-3">{!aiValidationResult && !isAiValidating && <div className="text-center text-xs text-gray-500 py-3"><p>Click for an extra layer of checks.</p></div>}{aiValidationResult?.errors?.length > 0 && <div className="rounded-md bg-red-50 p-3"><h4 className="text-xs font-bold text-red-800">Errors</h4><ul className="mt-2 text-xs text-red-700 list-disc pl-4 space-y-1">{aiValidationResult.errors.map((e) => <li key={e}>{e}</li>)}</ul></div>}{aiValidationResult?.warnings?.length > 0 && <div className="rounded-md bg-yellow-50 p-3"><h4 className="text-xs font-bold text-yellow-800">Warnings</h4><ul className="mt-2 text-xs text-yellow-700 list-disc pl-4 space-y-1">{aiValidationResult.warnings.map((w) => <li key={w}>{w}</li>)}</ul></div>}{aiValidationResult?.suggestions?.length > 0 && <div className="rounded-md bg-blue-50 p-3"><h4 className="text-xs font-bold text-blue-800">Suggestions</h4><ul className="mt-2 text-xs text-blue-700 list-disc pl-4 space-y-1">{aiValidationResult.suggestions.map((s) => <li key={s}>{s}</li>)}</ul></div>}</div>
            </div>
          </div>
