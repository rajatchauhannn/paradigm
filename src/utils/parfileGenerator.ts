...
export const generateParfileContent = (config: ParfileConfig): string => {
  const params = [];
  const { 
    userid, directory, dumpfile, logfile, parallel, operation, compression, 
    content, query, flashback_time, flashback_scn, export_mode, schemas, 
    tables, tablespaces, remap_schema, table_exists_action, version, 
    estimate_only, estimate, sqlfile 
  } = config;
...
export const generateCommand = (config: ParfileConfig, outputMode: 'parfile' | 'command'): string => {
  const opCommand = config.operation === 'EXPORT' ? 'expdp' : 'impdp';
  if (outputMode === 'parfile') return `nohup ${opCommand} parfile=your_parfile.par &`;

  const parfileLines = generateParfileContent(config).split('\n').filter(line => line.length > 0);
