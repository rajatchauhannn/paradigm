  ...
  return (
    <div>
      ...
      <div className="mt-4 space-y-4">
        ...
        <div>
          <label htmlFor="import_schemas" className="block text-sm font-medium text-gray-700">Schemas (optional filter)</label>
          <input id="import_schemas" type="text" placeholder="HR,SCOTT" value={config.schemas} onChange={(e) => setConfig(prev => ({...prev, schemas: e.target.value}))} className={inputClasses} />
        </div>
      </div>
    </div>
  );
