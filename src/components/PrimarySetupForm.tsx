...
export const PrimarySetupForm = ({ config, setConfig, isLogfileSame, onLogfileSameChange, onDumpfileChange }: PrimarySetupProps) => {
  return (
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
      ...
      <div>
        ...
        <input id="dumpfile" type="text" name="dumpfile" value={config.dumpfile} onChange={onDumpfileChange} className={inputClasses} />
      </div>
      ...
      <div>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center">
            <input id="logfileSameAsDumpfile" type="checkbox" className="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" checked={isLogfileSame} onChange={(e) => onLogfileSameChange(e.target.checked)} />
            <label htmlFor="logfileSameAsDumpfile" className="ml-2 block text-sm text-gray-600">Same as dumpfile?</label>
          </div>
        </div>
        <input id="logfile" type="text" name="logfile" value={config.logfile} onChange={(e) => setConfig(prev => ({ ...prev, logfile: e.target.value }))} className={`${inputClasses} ${disabledClasses}`} disabled={isLogfileSame} />
      </div>
    </div>
  );
};